...
> and the core `musetrainer-catalog.ts`). The catalog is now 8 of the app's own
> PD transcriptions in `packages/web/public/assets/library/`, generated by
> `scripts/gen-library-scores.mjs`, loaded by
> `packages/web/src/bundled-library.ts`, and validated end-to-end (parser + OSMD
> render). This doc is now the process for **growing** the set; the "shopping
> list" below is future repertoire.
