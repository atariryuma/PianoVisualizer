...
> and the core `musetrainer-catalog.ts`). The catalog is **20 of the app's own
> PD transcriptions** in `packages/web/public/assets/library/`, graded beginner
> → advanced (level 1–4), generated by `scripts/gen-library-scores.mjs`, loaded
> by `packages/web/src/bundled-library.ts` (sorted by level), and validated
> end-to-end (parser + OSMD render, pitches eyeballed). This doc is now the
> process for **growing** the set; the "shopping list" below is future
> repertoire.
