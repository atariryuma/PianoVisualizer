...
export function createOsmdCursor(deps: OsmdCursorDeps): OsmdCursor {
  ...
  function resetToStart(): void {
    ...
    // Container scroll-to-top dropped — `cursorsOptions.follow: true` lets
    // OSMD's own `cursor.reset()` invoke `scrollIntoView` on the iterator's
    // first note, so the container scrolls back to the start natively.
  }
